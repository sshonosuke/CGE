...
J <- 3    # number of effects
p <- 5    # number of covariates
n1 <- n2 <- n3 <- 2*round(sqrt(N))
m_set <- c(n1, n2, n3) 
ID <- cbind(sample(1:n1, N, replace=T), sample(1:n2, N, replace=T), sample(1:n3, N, replace=T))

# true coefficients
Beta <- c(1, -0.3, 0.3, rep(0, p-2))
...
if(scenario==2){
  V1 <- 0.2*(rgamma(n1, 1, 1) - 1)
  V2 <- 0.2*(1 - rgamma(n2, 1, 1)) 
  ch <- rbinom(n3, 1, 0.5)
  V3 <- ch*rnorm(n3, -0.3, 0.15) + (1-ch)*rnorm(n3, 0.3, 0.15)
}
V <- list(V1, V2, V3)

# Data generation 
X <- cbind(1, matrix(rnorm(N*p), N, p))
Mu <- as.vector(X%*%Beta) + V1[ID[,1]] + V2[ID[,2]] + V3[ID[,3]]
Lam <- exp(Mu)
Y <- rpois(N, Lam)
...
CPT <- c()
tt <- proc.time()[1]
fit_GE <- GE_CCD(Y=Y, X=X, ID=ID, m_set=m_set, G_set=round(sqrt(m_set)), family="poisson", tol=10^(-3), maxitr=200, print=F)
CPT[1] <- proc.time()[1] - tt
fit_GE$itr

# Mixed model 1
tt <- proc.time()[1]
fit_GLMM1 <- glmer(Y~X[,-1]+(1|ID[,1])+(1|ID[,2])+(1|ID[,3]), family="poisson", nAGQ=1)
CPT[2] <- proc.time()[1] - tt

# Mixed model 2
tt <- proc.time()[1]
fit_GLMM2 <- glmer(Y~X[,-1]+(1|ID[,1])+(1|ID[,2])+(1|ID[,3]), family="poisson", nAGQ=0)
...
# glmmTMB
data <- data.frame(Y=Y, X=X[,-1], ID1=ID[,1], ID2=ID[,2], ID3=ID[,3])
names(data)[2:(p+1)] <- paste0("X", 2:(p+1))
tt <- proc.time()[1]
fit_GLMM_tmb <- glmmTMB(Y~X2+X3+X4+X5+X6+(1|ID1)+(1|ID2)+(1|ID3), family=poisson, data=data)
...
# computation time
names(CPT) <- c("CGE", "MLE1", "MLE0", "TMB")
CPT


# Beta
Beta_est <- matrix(NA, p, 4)
dimnames(Beta_est)[[2]] <- c("CGE", "MLE1", "MLE0", "TMB")
...
# squared error loss
10^5*apply((Beta_est - Beta[-1])^2, 2, mean)
